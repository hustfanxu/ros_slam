...
RealTimeCorrelativeScanMatcher::GenerateExhaustiveSearchCandidates(
  ...
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
   ...
   // cout<<"num_candidates every step : "<<num_candidates<<endl;
  }

 // cout<<"search_parameters.num_scans : "<<search_parameters.num_scans<<endl;
...
double RealTimeCorrelativeScanMatcher::Match(
    ...
    transform::Rigid2d* pose_estimate,int16_t flag) const {
  ...
  const sensor::PointCloud2D rotated_point_cloud =
      ...
      rotated_point_cloud, probability_grid.limits().resolution(),flag);
