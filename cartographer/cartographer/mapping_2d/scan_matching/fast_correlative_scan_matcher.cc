...
bool FastCorrelativeScanMatcher::Match(
  ...
  const SearchParameters search_parameters(options_.linear_search_window(),
                                           ...
                                           point_cloud, limits_.resolution(),0);
...
bool FastCorrelativeScanMatcher::MatchFullSubmap(
  ...
  const transform::Rigid2d center = transform::Rigid2d::Translation(
   ...
   cout<<"size : "<<limits_.max()<<" "<< limits_.resolution()<<" "<<limits_.cell_limits().num_y_cells<<" "<<limits_.cell_limits().num_y_cells<<endl;
   cout<<"where is teh center  :  "<< center.translation()(0,0)<<" "<< center.translation()(1,0) <<endl;

  return MatchWithSearchParameters(search_parameters, center, point_cloud,
                                   min_score, score, pose_estimate);
}
bool FastCorrelativeScanMatcher::MatchPartSubmap(
    const sensor::PointCloud2D& point_cloud, float min_score, float* score,
    transform::Rigid2d* pose_estimate,transform::Rigid2d  my_center,int count_match_times) const {
  // Compute a search window around the center of the submap that includes it
  // fully.
  const SearchParameters search_parameters(
      45 *(1+count_match_times)* limits_.resolution(),  // Linear search window, 1e6 cells/direction.
      M_PI,  // Angular search window, 180 degrees in both directions.
      point_cloud, limits_.resolution(),0);
       const transform::Rigid2d center = transform::Rigid2d::Translation(
      Eigen::Vector2d(my_center.translation()(0,0)*limits_.resolution(), my_center.translation()(1,0)*limits_.resolution()));
   
//cout<<"size : "<<limits_.max()<<" "<< limits_.resolution()<<" "<<limits_.cell_limits().num_y_cells<<" "<<limits_.cell_limits().num_y_cells<<endl;
  // cout<<"where is the center  :  "<< center.translation()(0,0)<<" "<< center.translation()(1,0) <<endl;
...
FastCorrelativeScanMatcher::GenerateLowestResolutionCandidates(
  ...
  cout<<"linear_step_size  :  "<<linear_step_size<<endl;
