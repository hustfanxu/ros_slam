...
ProbabilityGrid ComputeCroppedProbabilityGrid(
  ...
  cout<<"limits.num_x_cells  :   "<<limits.num_x_cells<<"  "<<limits.num_y_cells<<endl;
...
proto::SubmapsOptions CreateSubmapsOptions(
  ...
  *options.mutable_laser_fan_inserter_options() = CreateLaserFanInserterOptions(
  ...
  LOG(INFO)<<"IIIIIIIIISSSSSSSSSSSSSS TTTTTTTTTHHHHHHHHHHHHHEEEEEEEEEERRRRRREEEEEE";
  return options;
}
...
void Submaps::InsertLaserFan(const sensor::LaserFan& laser_fan) {
  ...
  for (const int index : insertion_indices()) {
   //  cout<<"insertion_indices()   :   "<<index<<endl;
  ...
  //LOG(INFO)<<"options_.num_laser_fans()   :     "<<options_.num_laser_fans();
...
void Submaps::FinishSubmap(int index) {
 ...
 /*cout<<"probability_grid .x_max :  "<<   submap->probability_grid.get_max_x()<<endl;
  cout<<"probability_grid .y_max :  "<<   submap->probability_grid.get_max_y()<<endl;
  cout<<"probability_grid .x_min :  "<<   submap->probability_grid.get_min_x()<<endl;
  cout<<"probability_grid .y_min :  "<<   submap->probability_grid.get_min_y()<<endl;
*/
}
...
void Submaps::AddSubmap(const Eigen::Vector2f& origin) {
  ...
  LOG(INFO) << "origin " << origin(0,0)<<" "<<origin(1,0);
  LOG(INFO) << "Added submap " << size();
  LOG(INFO) << "options_.half_length()  :   " << options_.half_length();
  LOG(INFO) << "options_.resolution()  :   " << options_.resolution();
  LOG(INFO) << "num_cells_per_dimension  :   " << num_cells_per_dimension;
  LOG(INFO) << "num_laser_fans_ " << num_laser_fans_;
