...
class UnscentedKalmanFilter {
 public:
  ...
  int Observe(
    ...
    auto exam_delta =kalman_gain * -z_hat;
    Eigen::Vector3d exam_delta_vector(exam_delta[3],exam_delta[4],exam_delta[5]);
    double exam_delta_norm=exam_delta_vector.norm();
    if(exam_delta_norm>M_PI / 2.)
    {
      //  return -1;
	cout<<"the span is so   biggggggggggggggggggggggggggg"<<endl;
	cout<<"the span is so                big"<<endl;
	cout<<"the span is so                big"<<endl;
	cout<<"the span is so           bbbigggggggg        "<<endl;
	 cout<<"the span is so               big"<<endl;
	 cout<<"the span is so               big"<<endl;
	cout<<"the span is so   bigggggggggggggggggggggggggggg"<<endl;
    }
  ...
  const GaussianDistribution<FloatType, N>& GetBelief() const {
    ...
    void InsertBelief(GaussianDistribution<double, 9>& insert_belief)  {
     belief_= GaussianDistribution<FloatType, 9>(insert_belief.GetMean(), insert_belief.GetCovariance() );
  }
  template <int K>
  void Observe(
      std::function<Eigen::Matrix<FloatType, K, 1>(const StateType&)> h,
  ...
  //  cout<<"is the erroe here "<<endl;
    // Get the state mean and matrix root of its covariance.
    const StateType& mu = belief_.GetMean();
    const StateCovarianceType sqrt_sigma = MatrixSqrt(belief_.GetCovariance());
    cout<<"sqrt_sigma  :  "<<sqrt_sigma<<endl;
    // As in Kraft's paper, we compute W containing the zero-mean sigma points,
    // since this is all we need.
    std::vector<StateType> W;
    W.reserve(2 * N + 1);
    ...
    for (int i = 0; i < N; ++i) {
      ...
      // summation later on anyways.
      W.emplace_back(kSqrtNPlusLambda * sqrt_sigma.col(i));
     // cout<<"W 1: "<< W.back()<<endl;
      StateType temp1=W.back();
      Z.emplace_back(h(add_delta_(mu, temp1)));

      W.emplace_back(-kSqrtNPlusLambda * sqrt_sigma.col(i));
    //  cout<<"W 2: "<< W.back()<<endl;
      StateType temp2=W.back();
       Z.emplace_back(h(add_delta_(mu, temp2)));
      z_hat += kMeanWeightI * Z[2 * i + 1];
      z_hat += kMeanWeightI * Z[2 * i + 2];
    }
