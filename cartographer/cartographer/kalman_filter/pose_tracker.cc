...
ofstream pose_outfile;
ofstream pose_outfile_2;
string pose_name,pose_name_2;
...
void PoseTracker::InsertPoseEstimateMeanAndCovariance(const common::Time time,
                                                   transform::Rigid3d pose,
                                                   PoseCovariance  covariance) {
   Eigen::Matrix<double, 9, 1> insert_pose;
   Eigen::Vector3d insert_rotation_v =
      cartographer::transform::RotationQuaternionToAngleAxisVector(pose.rotation());
   insert_pose(0,0)=pose.translation()(0,0);
   insert_pose(1,0)=pose.translation()(1,0);
   insert_pose(2,0)=pose.translation()(2,0);
   insert_pose(3,0)=insert_rotation_v(0,0);
   insert_pose(4,0)=insert_rotation_v(1,0);
   insert_pose(5,0)=insert_rotation_v(2,0);   
   insert_pose(6,0)=0;
   insert_pose(7,0)=0;
   insert_pose(8,0)=0;
   insert_pose(9,0)=0;
   Eigen::Matrix<double, 9, 9> insert_covariance=Eigen::Matrix<double, 9, 9>::Zero();
   insert_covariance.block<6, 6>(0, 0)=covariance.block<6, 6>(0, 0);
  GaussianDistribution<double, 9> my_belief(insert_pose , insert_covariance);
  kalman_filter_.InsertBelief(my_belief);
...
void PoseTracker::AddPoseObservation(const common::Time timestamp,
                                     const transform::Rigid3d& pose,
                                     const PoseCovariance& covariance) {
  Predict(timestamp);
  Eigen::Matrix<double, 6, 1> gps_observation=Eigen::Matrix<double, 6, 1>::Zero();

   if (laser_tiemstamp_first==0&&gps_tiemstamp_first==0&&(gps_tracker.gps_flag==3||gps_tracker.gps_flag==4))
   {
       gps_tiemstamp_first=gps_tracker.time;
       laser_tiemstamp_first=timestamp.time_since_epoch().count();
       system_clock::time_point now = system_clock::now();
       time_t t = system_clock::to_time_t(now);
       char ch[32];
        strftime(ch, sizeof(ch), "%Y_%m_%d %H_%M_%S", localtime(&t));
        cout<<"store pose data------------>>>>>>>>>>>>>>>---------------->>>>>>>>>>>>>>>"<<endl;

        pose_name=ch;
        pose_name_2=ch;
        pose_name="/home/wxl/ros_slam/catkin_ws/src/cartographer_ros/my_package/src/data/pose_"+pose_name+".txt";
        pose_name_2="/home/wxl/ros_slam/catkin_ws/src/cartographer_ros/my_package/src/data/pose_"+pose_name_2+"_2"+".txt";
        pose_outfile.open(pose_name);
        pose_outfile_2.open(pose_name_2);
        pose_outfile.close();
        pose_outfile_2.close();
   }
   int time_stamp_1=timestamp.time_since_epoch().count()-laser_tiemstamp_first;
  ...
  if(abs(delta_t)<300&&(gps_tracker.gps_flag==3||gps_tracker.gps_flag==4))
  {
       pose_outfile.open(pose_name);
       pose_outfile_2.open(pose_name_2);
       pose_outfile<< pose.translation()(0,0)<<" "<< pose.translation()(1,0)<<" "<<time_stamp_1<<"\n";
       pose_outfile_2<< gps_tracker.gps_pose.translation()(0,0)<<" "<< gps_tracker.gps_pose.translation()(1,0)<<" "<<gps_tracker.gps_flag<<" "<<time_stamp_2<<"\n";
       pose_outfile.close();
       pose_outfile_2.close();
  }
    ...
    const cartographer::transform::Rigid2d& gps_data_covariance,int32 gps_flag)
{
   ...
   cout<<"get gps data in pose tracker"<<endl;
    cartographer::transform::Rigid2d recieve_pose_xy=cartographer::transform::Rigid2d::Translation(Eigen::
                              Vector2d(gps_data.translation()(0,0),gps_data.translation()(1,0)));
    cartographer::transform::Rigid2d recieve_pose_angle=cartographer::transform::Rigid2d::Rotation(gps_data.rotation().angle());

    cartographer::transform::Rigid2d gps_xy_1=cartographer::transform::Rigid2d(recieve_pose_xy.translation(),gps_data.rotation());


//    gps_tracker.gps_pose=transform::Rigid2d(gps_xy_1.translation(),gps_xy_1.rotation());
